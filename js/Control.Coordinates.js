...
L.Control.Coordinates = L.Control.extend({
	...
	setCoordinates: function(obj)
	{
		...
		if (obj.latlng) {
			if (obj.latlng.lat < 0 || obj.latlng.lat > 1000 || obj.latlng.lng < 0 || obj.latlng.lng > 1000) {
				L.DomUtil.addClass(this._container, 'hidden');
			} else {
				L.DomUtil.removeClass(this._container, 'hidden');
			}

			L.DomUtil.get(this._lat).innerHTML = '<strong>' + this.options.latitudeText + ':</strong> ' + ((1000 - obj.latlng.lat)/10).toFixed(this.options.precision).toString();
			L.DomUtil.get(this._lng).innerHTML = '<strong>' + this.options.longitudeText + ':</strong> ' + (obj.latlng.lng / 10).toFixed(this.options.precision).toString();
		}
	}
});